...
mod tests {
    ...
    fn ps_is_always_shell_command() {
        // The `ps`/`ps5` block keyword has been deleted in favor of
        // `plugin.ps ... end`. Every form of `ps` is now the unix
        // command and must submit cleanly without entering multi-
        // line capture mode.
